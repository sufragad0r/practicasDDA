...
public class ScoreActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ActivityScoreBinding binding =
                ActivityScoreBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());
        int localScore = getIntent().getExtras().getInt(LOCAL_SCORE_KEY);
        int visitorScore =
                getIntent().getExtras().getInt(VISITOR_SCORE_KEY);

        binding.scoreText.setText(getString(R.string.local_visitor_score_format,
                localScore,
                visitorScore));
